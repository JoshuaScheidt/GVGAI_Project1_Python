...
import math
from encoderGeneralFunctions import readJsonDataAutoEncode, loadModel, loadModelWeights, readFolderDataAutoEncode
...
if(DATAPATH[len(DATAPATH)-4:len(DATAPATH)] ==".txt"):
    (inputObject) = readJsonDataAutoEncode(DATAPATH)
elif Path(DATAPATH).exists():
    (inputObject) = readFolderDataAutoEncode(DATAPATH)
else:
...
modelRepresentation = "_"+data+"_auto_"
...
if os.path.exists(DATAPATH):
    ...
    model.fit(inputObject, inputObject, epochs=epochs, batch_size=1, verbose=1, callbacks=[history])
